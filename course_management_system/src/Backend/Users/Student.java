...
public class Student extends User {
    ...
    public void enrollModules(String moduleName) {
        try {
            ...
            enrollModulesSt.executeUpdate("UPDATE STUDENT_ENROLLMENT SET STATUS='ENROLLED' WHERE STUDENT_NAME='"
                    + username + "' AND MODULE_NAME='" + moduleName + "';");
    ...
    public void fetchEnrollmentTableData(JTable table) {
        String query = "SELECT ID,STUDENT_NAME,COURSE_NAME,MODULE_NAME,MODULE_TYPE,STATUS FROM STUDENT_ENROLLMENT WHERE COURSE_NAME='"
                + getCourseName() + "' AND STUDENT_NAME='" + username + "';";
        DefaultTableModel model = (DefaultTableModel) table.getModel();
        model.setRowCount(0);
        try {
            Statement st = connection.createStatement();
